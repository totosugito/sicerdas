...
6. **Variable Formulas (CRITICAL for math):** Always parameterize math questions! Do not leave `variableFormulas: null` for calculation questions. Scan the equations for constants (e.g., base numbers like 2020, coefficients like 3, evaluated points like $f(2)$). Replace these constants with placeholders like `{{a}}`, `{{b}}` inside the LaTeX strings (e.g., `\log_{{{a}}} ({{b}}x)`). Generate 3-5 distinct sets of valid values in `variableFormulas.variables`, ensuring mathematical rules (like log domains) remain valid. Include `opt1`, `opt2`, etc., in the variables.
...
3. **Step-by-step Numbering & Detailed Reasoning** — Break the logical sequence into numbered steps (using a standard `paragraph` block with a bold number like `"1. "`). Each step MUST have a detailed explanation. This applies to BOTH conceptual solutions and fast methods. Do not just drop a formula; explain *why* and *how* it applies.
4. **Concept Reminders (Callouts)** — Insert an `alert` block (`type: "info"`) before a calculation to highlight a formula. The alert must be extremely concise. **CRITICAL JSON FORMAT:** If the alert contains a math formula, you MUST split the `content` array. Do not put raw LaTeX inside the `text` string! Example: `content: [{"type": "text", "text": "💡 Ingat Sifat: "}, {"type": "latex", "props": {"latex": "a^{\\log_a b} = b", "displayMode": false}}]`
