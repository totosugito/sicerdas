...
- 17. **`exam_package_interactions`**: Per-user interaction ledger. Tracks individual state for each package, including `liked`, `bookmarked`, `rating`, and `viewCount`. Additionally tracks the user's **`completionStatus`** (NOT_STARTED, IN_PROGRESS, COMPLETED) and **`completedSectionsCount`** for multi-section packages.
