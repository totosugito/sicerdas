...
export function GradeTable({
  ...
  const columns: CustomColumnDef<GradeData>[] = [
    ...
    }),
    {
      ...
      cell: ({ row }) => <span className="font-medium">{row.getValue("grade")}</span>,
    },
    {
      accessorKey: "name",
      enableSorting: true,
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t(($) => $.education.grade.table.columns.name)}
        />
      ),
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t(($) => $.education.grade.table.columns.desc)}
        />
      ),
      cell: ({ row }) => (
        <LongText
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t(($) => $.education.grade.table.columns.updatedAt)}
        />
      ),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader
          column={column}
          title={t(($) => $.education.grade.table.columns.actions)}
          className="justify-center"
        />
      ),
      cell: ({ row }) => {
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="h-8 w-8 p-0">
                  <span className="sr-only">
                    {t(($) => $.education.grade.table.actions.openMenu)}
                  </span>
