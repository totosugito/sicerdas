...
function RouteComponent() {
  ...
  const updateUrlParams = (
    newPage?: number,
    newSearch?: string,
    newFilters?: { categories: number[]; groups: number[]; grades?: number[] },
    newSortBy?: string,
  ...
  const isAnyFilterActive = !!(
    (selectedFilters.categories.length > 0 &&
      !(selectedFilters.categories.length === 1 && selectedFilters.categories[0] === 0)) ||
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            {!isLoading && books.length > 0 && (
              <BookCard books={books} viewMode={viewMode === "grid" ? "grid" : "list"} />
            )}
          </div>
