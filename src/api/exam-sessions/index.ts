    ...
    return useMutation({
        ...
        mutationFn: async (packageId: string) => {
            const response = await fetchApi({
                method: "POST",
                url: AppApi.exam.sessions.client.start,
                body: { packageId },
...
export const useSessionDetails = (id: string, enabled = true) => {
    return useQuery({
        ...
        queryFn: async () => {
            const response = await fetchApi({
                method: "GET",
                url: AppApi.exam.sessions.client.details.replace(":id", id),
                withCredentials: true,
            });
...
export const useSaveAnswer = () => {
    return useMutation({
        ...
        mutationFn: async (body: SaveAnswerRequest) => {
            const response = await fetchApi({
                method: "POST",
                url: AppApi.exam.sessions.client.saveAnswer,
                body,
                withCredentials: true,
            });
...
export const useSubmitSession = () => {
    return useMutation({
        ...
        mutationFn: async (id: string) => {
            const response = await fetchApi({
                method: "POST",
                url: AppApi.exam.sessions.client.submit.replace(":id", id),
                withCredentials: true,
            });
            return response as CommonResponse;
        },
