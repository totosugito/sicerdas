...
} from "@/features/exam/question-solutions/list-solution";
...
export function QuestionSolutionsTab({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      ...
      Promise.all(
        ...
        .then(() => {
          showNotifSuccess({ message: t(($) => $.exam.solutions.orderSuccess) });
          queryClient.invalidateQueries({ queryKey: ["admin-exam-question-detail"] });
        })
        .catch((error: any) => {
          showNotifError({ message: error.message || t(($) => $.exam.solutions.orderError) });
          queryClient.invalidateQueries({ queryKey: ["admin-exam-question-detail"] });
        });
    }
  };
  ...
  const confirmDelete = () => {
    ...
    deleteMutation.mutate(solutionToDelete, {
      onSuccess: (res) => {
        showNotifSuccess({
          message: res.message || t(($) => $.labels.delete) + " " + t(($) => $.labels.active),
        });
        queryClient.invalidateQueries({ queryKey: ["admin-exam-question-detail"] });
      ...
      <CardContent className="space-y-0">
        {items.length > 0 ? (
          ...
          <div className="text-center py-12 border-2 border-dashed rounded-2xl bg-muted/5">
            <p className="text-muted-foreground italic">{t(($) => $.exam.solutions.empty)}</p>
          </div>
        )}
        ...
        <DialogModal
          ...
          modal={{
            title: t(($) => $.exam.solutions.delete.confirmTitle),
            desc: t(($) => $.exam.solutions.delete.confirmDesc),
            variant: "destructive",
            iconType: "error",
            textConfirm: deleteMutation.isPending
              ? t(($) => $.labels.saving)
              : t(($) => $.labels.delete),
