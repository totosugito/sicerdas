...
export const DialogQuestionSolutionForm = ({ open, onOpenChange, questionId, solution, nextOrder }: DialogQuestionSolutionFormProps) => {
    ...
    const formSchema = {
        title: z.string().min(1, t($ => $.labels.required)),
        content: z.array(z.record(z.string(), z.unknown()))
            .min(1, t($ => $.labels.required))
            .refine(val => blocknote_to_text(val).trim().length > 0, {
                message: t($ => $.labels.required)
            }),
        solutionType: z.enum(Object.values(EnumSolutionType) as [string, ...string[]]).default(EnumSolutionType.GENERAL),
    ...
    const formConfig = {
        ...
        solutionType: {
            ...
            options: [
                { label: t($ => $.exam.solutions.form.type.options.general), value: EnumSolutionType.GENERAL },
                { label: t($ => $.exam.solutions.form.type.options.fast_method), value: EnumSolutionType.FAST_METHOD },
                { label: t($ => $.exam.solutions.form.type.options.video_link), value: EnumSolutionType.VIDEO_LINK },
    ...
    const modalProps: ModalFormProps = {
        title: solution ? t($ => $.exam.solutions.dialog.editTitle) : t($ => $.exam.solutions.dialog.addTitle),
        desc: solution ? t($ => $.exam.solutions.dialog.editDescription) : t($ => $.exam.solutions.dialog.createDescription),
        modal: true,
        textConfirm: (createMutation.isPending || updateMutation.isPending) ? t($ => $.labels.saving) : t($ => $.labels.save),
        textCancel: t($ => $.labels.cancel),
        defaultValue: {
            title: solution?.title || "",
            content: solution?.content && solution.content.length > 0 ? solution.content : [{ type: "paragraph", content: [] }],
