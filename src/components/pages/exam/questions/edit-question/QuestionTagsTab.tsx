                    ...
                    <div className="space-y-3">
                        ...
                        <Popover open={open} onOpenChange={setOpen}>
                            <PopoverTrigger asChild>
                                <button className="relative group w-full text-left outline-none">
                                    <div className={cn(
                                        "h-14 w-full bg-background rounded-2xl border-2 transition-all flex items-center px-5 gap-4 shadow-sm",
                                        open ? "border-primary ring-4 ring-primary/5" : "border-muted hover:border-primary/30"
                                    )}>
                                        <Search className={cn("h-5 w-5 transition-colors", open ? "text-primary" : "text-muted-foreground/50")} />
                                        <span className="h-8 w-px bg-muted-foreground/10"></span>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="h-8 w-8 rounded-xl bg-muted/50 flex items-center justify-center text-[10px] font-bold text-muted-foreground tabular-nums opacity-60">
                                                ⌘K
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0 rounded-2xl overflow-hidden border-2 shadow-2xl" align="start" sideOffset={8}>
                            ...
                            {tags && tags.length > 0 ? (
                                tags.map(tag => (
                                    <Badge
                                        key={tag.id}
                                        variant="secondary"
                                        className="pl-4 pr-1.5 py-1.5 rounded-2xl bg-primary/5 hover:bg-primary/10 text-primary border-primary/10 gap-3 group/badge transition-all hover:scale-[1.02] active:scale-95 cursor-default shadow-sm"
                                    >
                                        <span className='font-semibold text-sm tracking-tight'>{tag.name}</span>
                                        <button
                                            onClick={() => handleUnassign(tag.id)}
                                            disabled={unassignMutation.isPending}
                                            className="h-6 w-6 rounded-full bg-primary/10 flex items-center justify-center text-primary hover:bg-red-500 hover:text-white transition-all transform group-hover/badge:scale-110 active:scale-90 disabled:opacity-50"
                                        >
                                ...
                                <div className="flex flex-col items-center justify-center w-full py-4 text-muted-foreground/40 border-2 border-dashed border-muted/50 rounded-2xl bg-muted/5">
                                    ...
                                    <span className="text-xs italic font-medium">{t($ => $.exam.tags.empty || "Belum ada tag terpilih")}</span>
                                </div>
                            )}
                        </div>
                    </div>
