    ...
    try {
      ...
      if (schema) {
        ...
        try {
          ...
          return {
            formErrors: [zodError.message || t(($) => $.labels.formValidationErrorGeneric)],
          };
        }
      }
      return null;
  ...
  const validateForm = async () => {
    try {
      ...
      if (!isValid && Object.keys(formErrors).length === 0) {
        ...
        return {
          isValid: false,
          errors: zodErrors || { formErrors: [t(($) => $.labels.formValidationError)] },
        };
      }
      ...
      return {
        isValid: false,
        errors: { formErrors: [t(($) => $.labels.formValidationErrorUnknown)] },
      };
    }
  };
    ...
    try {
      ...
      if (!isValid) {
        ...
        if (errors) {
          ...
          if ("fieldErrors" in errors && !("message" in errors)) {
            // check !message to distinguish from FieldError that might have message prop
            ...
            if (fieldErrors && typeof fieldErrors === "object") {
              const errorMessages = Object.entries(fieldErrors).map(([fieldName, ferrs]) => {
