...
export const getBookCoverUrl = ({
  ...
  size.forEach((s: string) => {
    urls[s] =
      `${env.server.s3Storage.publicUrl}/book/images/${dirName}/${s}/${fileId}/${fileId}_0000_${s}.jpg`;
  });
  return urls;
};

export const getBookSamplePagesUrl = ({
  bookId,
  maxChar = 4,
  size = ["xs", "lg"],
}: {
  ...
  const urls: Record<string, string> = {};
  size.forEach((s) => {
    urls[s] = `${env.server.s3Storage.publicUrl}/book/images/${dirName}/${s}/${fileId}/${fileId}_`;
  });
  return urls;
};
...
export const getBookPdfUrl = ({ bookId, maxChar = 4 }: { bookId: number; maxChar?: number }) => {
  const dirName = Math.floor(bookId / 1000);
  return `${env.server.s3Storage.publicUrl}/book/pdf/${dirName}/${createAutoBookName(bookId, maxChar)}.pdf`;
};
