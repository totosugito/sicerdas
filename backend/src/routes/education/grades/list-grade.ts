...
const EducationGradeListQuery = Type.Object({
  ...
  sortBy: Type.Optional(
    Type.String({
      description: "Sort field: createdAt, updatedAt, name, grade",
      default: "updatedAt",
    }),
  ),
    ...
    handler: withErrorHandler(async function handler(
      ...
      switch (sortBy) {
        case "name":
          queryWithSort =
            orderDir === "asc"
              ? baseQuery.orderBy(asc(educationGrades.name))
              : baseQuery.orderBy(desc(educationGrades.name));
          break;
        case "grade":
          queryWithSort =
            orderDir === "asc"
              ? baseQuery.orderBy(asc(educationGrades.grade))
              : baseQuery.orderBy(desc(educationGrades.grade));
          break;
        case "updatedAt":
          queryWithSort =
            orderDir === "asc"
              ? baseQuery.orderBy(asc(educationGrades.updatedAt))
              : baseQuery.orderBy(desc(educationGrades.updatedAt));
          break;
        case "createdAt":
          queryWithSort =
            orderDir === "asc"
              ? baseQuery.orderBy(asc(educationGrades.createdAt))
