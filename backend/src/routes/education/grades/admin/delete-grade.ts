...
const deleteEducationGradeRoute: FastifyPluginAsyncTypebox = async (app) => {
    app.route({
        ...
        handler: async function handler(
            ...
            if (!existingGrade) {
                return reply.notFound(request.t($ => $.education.grades.delete.notFound));
            }
            ...
            if (bookUsage) {
                return reply.badRequest(request.t($ => $.education.grades.delete.inUseBook));
            }
            ...
            if (packageUsage) {
                return reply.badRequest(request.t($ => $.education.grades.delete.inUsePackage));
            }
            ...
            if (questionUsage) {
                return reply.badRequest(request.t($ => $.education.grades.delete.inUseQuestion));
            }
