...
class Trick88DivideBy8 extends BaseTrick {
  final String chapterKey;
  late int op1;
  late int op2;

  Trick88DivideBy8(super.level, {required this.chapterKey});
  ...
  String createHtmlStepsSolution(BuildContext context) {
    ...
    steps += createStepLabel(
      stepNo: 1,
      text: l10n.math_tricks.trick88.step1(divisor1: var1.toString()),
    );
    ...
    steps += createStepLabel(
      stepNo: 2,
      text: l10n.math_tricks.trick88.step2(temp1: temp1.toString(), divisor2: var2.toString()),
    );
    steps += createStepValue(
      text:
          "${spanColor(temp1, 'green')}${divide()}${spanColor(var2, 'magenta')}${equal()}${spanColor(temp2, 'cyan')}",
    );

    // Step 3: Divide result by 2
    steps += createStepLabel(
      stepNo: 3,
      text: l10n.math_tricks.trick88.step3(temp2: temp2.toString(), divisor3: var3.toString()),
    );
    steps += createStepValue(
      text:
          "${spanColor(temp2, 'cyan')}${divide()}${spanColor(var3, 'orange')}${equal()}${spanColor(answerStr, 'default')}",
    );
