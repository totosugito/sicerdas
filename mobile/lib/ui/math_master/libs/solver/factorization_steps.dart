...
class FactorizationSteps extends LibHtml {
  ...
  String _htmlTreeFactor(String label, int checkNumber, List<int> tree) {
    String html = span(
      id: idStepsLabel,
      value: sprintf(label, [b(checkNumber.toString()), br()]),
    );
    ...
    for (int i = 2; i < tree.length; i += 2) {
      ...
      htmlTree += sprintf("%s%s%s%d%s\n", [
        nSpace(spaceL - rnNum),
        b(lNum.toString()),
        nSpace(shortSpace + rnNum),
        rNum,
        br(),
      ]);
      spaceL = spaceR;
  ...
  String _htmlPrimeFactorization(
    String label,
    int checkNumber,
    List<int> tree,
    Map<int, int> group,
  ...
  String _htmlGetFactorization(String label, Map<int, int> data, int result) {
    ...
    html += span(
      id: idStepsValue,
      value: mapRankIntToString(data: data, separator: times(), showAll: false),
    );
    ...
    String html = span(
      ...
      value: sprintf(label, [
        listNumbersIntToString(
          data: numbers,
          lastSeparator: sprintf(" %s ", [t.math_master.solver.and_text]),
        ),
        br(),
      ]),
    );
  ...
  String toHtmlGcf() {
    ...
    for (int i = 0; i < numbers.length; i++) {
      ...
      html += liSpan(
        value: _htmlTreeFactor(label1, numbers[i].getValI(), trees[i]),
      );

      // 2. prime factorization
      html += liSpan(
        value: _htmlPrimeFactorization(
          label2,
          numbers[i].getValI(),
          trees[i],
          treeGroup[i],
