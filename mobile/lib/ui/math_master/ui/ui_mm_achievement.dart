...
class UiMmAchievement extends ConsumerStatefulWidget {
  ...
  static void navigate(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const UiMmAchievement(),
      ),
    );
  }
...
class _UiMmAchievementState extends ConsumerState<UiMmAchievement> {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      chapterOpened = opened == 0 ? 1 : opened; // Minimum 1 chapter is accessible
      chapterTotal = allChapters.length;
      todayCorrect = todaySummary['correct'] ?? 0;
      todayWrong = todaySummary['wrong'] ?? 0;
      achievementList = DataMathMaster.createAchievementList(context, allChapters);
      _isLoading = false;
    });
  }
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: Icon(Icons.today_rounded, color: theme.colorScheme.primary),
                        ),
                        const SizedBox(width: 16),
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(locale.today_score, style: theme.textTheme.large.copyWith(fontWeight: FontWeight.bold)),
                              const SizedBox(height: 4),
                              Row(
                                children: [
                                  Text(
                            ...
                            ...parent.child.map((chapter) {
                              return Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Text(chapter.title, style: theme.textTheme.muted),
                                    Row(
                                      children: [
                                        Text(
